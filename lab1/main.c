...
int print_last(int num_row, FILE*fp, int ID, int quan) {
    for(int i = quan; i < 5; i++) {
        fprintf(fp,"[%d]: " , i);
        for(int j = 0; j < 7; j++) {
            fprintf(fp, "-- ");
            j == 6 ? fprintf(fp,"\n") : 1;
...
int main() {
	...
	scanf("%d", &quan);
	printf("已為您購買的%d組樂透組合輸出至lotto[%05d].txt", quan, sell_numpluse[0]);
    fprintf(fp, "======== lotto649 =========\n");
    ...
    print_last(row_num, fp, ID, quan); //fprintf4
    fclose(fp);
    return 0;
}
